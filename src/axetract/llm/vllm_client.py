...
class LocalVLLMClient(BaseClient):
    def __init__(self, config: dict):
        ...
        for name, data in self.lora_config_raw.items():
            ...
            defaults = {} if isinstance(data, str) else {k: v for k, v in data.items() if k != "path"}
            
            self.lora_requests[name] = LoRARequest(lora_name=name, lora_int_id=self._lora_id_counter, lora_path=path)
    ...
    def _create_sampling_params(self, adapter_name: str = None, **kwargs) -> SamplingParams:
        params = {"temperature": self.temperature, "top_p": self.top_p, "max_tokens": self.max_tokens, "stop": self.stop_sequences}
        if adapter_name and adapter_name in self.adapter_defaults:
            params.update(self.adapter_defaults[adapter_name])
        params.update({k: v for k, v in kwargs.items() if v is not None})
        
        safe_truncate_len = max(self.context_window_size - params["max_tokens"] - 1, self.context_window_size // 2)
        return SamplingParams(
            temperature=params["temperature"], top_p=params["top_p"], max_tokens=params["max_tokens"], 
            stop=params["stop"], truncate_prompt_tokens=safe_truncate_len
        )

    def call_batch(self, prompts: Iterable[str], adapter_name: str = None, thinking: bool = False, **kwargs) -> List[Optional[str]]:

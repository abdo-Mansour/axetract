...
class SmartHTMLProcessor:
    """Processor for extracting and manipulating HTML content chunks while maintaining structure."""

    def __init__(self):
        """Initialize the processor with relevant HTML tag sets."""
    ...
    def _clean_element(self, element, is_root=False):
        etree.strip_elements(
            element, "script", "style", "meta", "noscript", "link", "svg", "iframe"
        )
    ...
    def _is_layout_table(self, element):
        ...
        if element.find(".//table") is not None:
            return True
        if len(element.findall(".//tr")) < 2:
            return True
        return False

    def extract_chunks(self, html_content):
        """Split HTML into chunks of text and atomic elements.

        Args:
            html_content (str): The raw HTML to chunk.

        Returns:
            List[dict]: Extracted chunks with XPaths and content.
        """
        ...
        def flush_buffer(element_xpath):
            ...
            if clean and (len(clean) > 1 or "<img" in raw):
                chunks.append(
                    {"id": counter["id"], "xpath": element_xpath, "content": clean, "type": "text"}
                )
                counter["id"] += 1
            buffer.clear()
        ...
        def traverse(element):
            ...
            for child in element:
                ...
                if is_layout or (
                    tag not in self.BLOCK_TAGS
                    and tag not in self.ATOMIC_TAGS
                    and tag not in self.INLINE_TAGS
                ):
                ...
                if is_block or is_atomic:
                    ...
                    if is_atomic:
                        ...
                        clean_html = self._normalize_whitespace(
                            self._element_to_string(child_copy), preserve_newlines=(tag == "pre")
                        )
                        ...
                        has_block_children = any(
                            (c.tag in self.BLOCK_TAGS or c.tag in self.ATOMIC_TAGS)
                            and not self._is_layout_table(c)
                            for c in child
                        )
                        if has_block_children:
                            ...
                            self._clean_element(child_copy, is_root=True)
                            if (
                                bool(child_copy.text_content().strip())
                                or child_copy.find(".//img") is not None
                            ):
                                clean_html = self._normalize_whitespace(
                                    self._element_to_string(child_copy)
                                )
                                chunks.append(
                                    {
                                        "id": counter["id"],
    ...
    def reconstruct_skeleton(self, original_html, selected_chunks):
        """Reconstruct a structural HTML skeleton containing only selected chunks.

        Args:
            original_html (str): The full original HTML document.
            selected_chunks (List[dict]): The chunks to keep in the skeleton.
        ...
        def prune(element, parent_is_selected=False):
            ...
            should_keep_text = is_explicitly_selected or (
                parent_is_selected and element.tag in self.INLINE_TAGS
            )
...
def normalize_html_text(text: str) -> str:
    """Normalize text by converting whitespace and collapsing spaces.
...
def fetch_content(url: str, timeout=15) -> str:
...
def clean_html(
    ...
    Args:
        ...
        strip_attrs (bool, optional): If True, remove all tag attributes (e.g., class, id).
                                      Defaults to True.
        strip_links (bool, optional): If True, replace <a> tags with their inner text.
                                      Defaults to True.
...
def chunk_html_content(
    html_content: str, max_tokens: int = 500, is_clean: bool = True, attr_cutoff_len: int = 5
) -> List[str]:
...
def find_closest_html_node(html_text, search_text):
    """Return the chunk (and its xpath/sub_index) that matches the search query.

    Matches based on:
      - includes the normalized search_text as substring, and
      - has the highest fuzzy score among all such chunks.

    If no chunk includes search_text, returns:
        {'text': search_text, 'xpath': None, 'sub_index': None, 'score': 0.0, 'found': False}.
    """
...
def extract_visible_xpaths_leaves(
    cleaned_html: str, min_length: int = 1, dedupe_texts: bool = True
) -> List[Tuple[str, str]]:
            ...
            if child.tail:
                ...
                if len(tail_text) >= min_length:
                    if not (dedupe_texts and tail_text in seen_texts):
                        # Note: The text physically follows 'child', but structurally belongs to
                        # 'el' (parent)
...
def merge_xpaths_to_html(
    xpath_text_list: List[Tuple[str, str]], root_tag: str = "html", pretty: bool = True
) -> str:
        ...
        for frag in frags:
            if isinstance(frag, str):
                if len(cur) > 0:
                    last_child = cur[-1]
                    last_child.tail = (
                        (last_child.tail or "") + (" " if last_child.tail else "") + frag
                    )
                else:
